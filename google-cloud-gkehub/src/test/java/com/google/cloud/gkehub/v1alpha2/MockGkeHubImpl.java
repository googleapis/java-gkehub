...
import com.google.cloud.gkehub.v1alpha2.GkeHubGrpc.GkeHubImplBase;
...
public class MockGkeHubImpl extends GkeHubImplBase {
  ...
  public void initializeHub(
      InitializeHubRequest request, StreamObserver<InitializeHubResponse> responseObserver) {
    Object response = responses.poll();
    if (response instanceof InitializeHubResponse) {
      requests.add(request);
      responseObserver.onNext(((InitializeHubResponse) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method InitializeHub, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  InitializeHubResponse.class.getName(),
                  Exception.class.getName())));
    }
  }
}
